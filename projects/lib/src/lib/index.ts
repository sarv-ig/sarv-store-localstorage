...
const validateStateKeys = (keys: Keys) => {
    return (keys as any[]).map((key) => {
        ...
        if (typeof attr !== 'string') {
            throw new TypeError(
                `sarvLocalStorageSync Unknown Parameter Type: ` + `Expected type of string, got ${typeof attr}`
            );
        }
        return key;
    });
...
export const sarvLocalStorageSync = (config: LocalStorageConfig) => (reducer: any) => {
    if (
